...
/**
 * Cross-Encoder 重排策略：用 DashScope 的 qwen3-rerank 专用重排模型（cross-encoder）。
 * <p>
...
public class CrossEncoderRerankStrategy implements RerankStrategy {
    ...
    /**
     * rerank 模型名。DashScope 推荐使用 qwen3-rerank（gte-rerank-v2 已于 2026 年 5 月 30 日下线）
     */
    private final String model;

    public CrossEncoderRerankStrategy(ObjectProvider<RerankModel> rerankModelProvider,
                                      @Value("${app.rag.reranker.cross-encoder-model:qwen3-rerank}") String model) {
